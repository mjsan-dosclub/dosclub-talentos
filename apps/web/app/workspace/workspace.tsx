...
type Session={id:string;group_id:string;title:string;objective:string;venue:string;mode:string;starts_at:string;ends_at:string;closed_at:string|null;institution:string;group_name:string;evidence_required:boolean;evidence_types:string[];evidence_deadline:string|null};
type Data={
...
export function Workspace(){
  ...
  return <>
    ...
    <section className="workspace-section"><div className="workspace-section-title"><p className="eyebrow">SESSIONS</p><h2>The work in motion.</h2><p className="section-hint">Select a session title to preview its details.</p></div>{data.sessions.length?<div className="data-list">{data.sessions.map(s=><article key={s.id}><time>{new Date(s.starts_at).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'})}</time><div><button className="session-link" onClick={()=>setSelectedSession(s)}>{s.title}</button><p>{s.institution} · {s.group_name} · {s.mode}</p><small>{s.evidence_required?`Evidence due ${new Date(s.evidence_deadline!).toLocaleString('en-IN')}`:'No evidence required'}</small></div></article>)}</div>:<p className="empty-copy">No sessions are available in your scope yet.</p>}</section>
    {selectedSession&&<div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setSelectedSession(null)}}><section className="modal-panel" role="dialog" aria-modal="true"><button className="modal-close" onClick={()=>setSelectedSession(null)} aria-label="Close">×</button><p className="eyebrow">{selectedSession.mode} · {selectedSession.group_name}</p><h3>{selectedSession.title}</h3><p>{selectedSession.objective}</p><p><strong>When:</strong> {new Date(selectedSession.starts_at).toLocaleString('en-IN')} – {new Date(selectedSession.ends_at).toLocaleString('en-IN')}</p><p><strong>Venue or meeting reference:</strong> {selectedSession.venue}</p><p><strong>Evidence:</strong> {selectedSession.evidence_required?`${selectedSession.evidence_types.join(', ')} · due ${new Date(selectedSession.evidence_deadline!).toLocaleString('en-IN')}`:'No evidence required'}</p></section></div>}
